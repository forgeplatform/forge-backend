...
"""
Unified Job and Unified Job Template serializers.

These are base serializers for polymorphic job types (Job, ProjectUpdate, InventoryUpdate, etc.)
...
class UnifiedJobTemplateSerializer(BaseSerializer):
    ...
    def get_sub_serializer(self, obj):
        """
        Returns the appropriate serializer class for a polymorphic object.
        ...
        if type(self) is UnifiedJobTemplateSerializer:
            ...
            if isinstance(obj, Project):
                from forge.api.serializers import ProjectSerializer
                serializer_class = ProjectSerializer
            elif isinstance(obj, InventorySource):
                from forge.api.serializers import InventorySourceSerializer
                serializer_class = InventorySourceSerializer
            elif isinstance(obj, JobTemplate):
                from forge.api.serializers import JobTemplateSerializer
                serializer_class = JobTemplateSerializer
            elif isinstance(obj, SystemJobTemplate):
                from forge.api.serializers import SystemJobTemplateSerializer
                serializer_class = SystemJobTemplateSerializer
            elif isinstance(obj, WorkflowJobTemplate):
                from forge.api.serializers import WorkflowJobTemplateSerializer
                serializer_class = WorkflowJobTemplateSerializer
            elif isinstance(obj, WorkflowApprovalTemplate):
                from forge.api.serializers import WorkflowApprovalTemplateSerializer
...
class UnifiedJobSerializer(BaseSerializer):
    ...
    def get_sub_serializer(self, obj):
        ...
        if type(self) is UnifiedJobSerializer:
            # Lazy imports to avoid circular dependencies
            if isinstance(obj, ProjectUpdate):
                from forge.api.serializers import ProjectUpdateSerializer
                serializer_class = ProjectUpdateSerializer
            elif isinstance(obj, InventoryUpdate):
                from forge.api.serializers import InventoryUpdateSerializer
                serializer_class = InventoryUpdateSerializer
            elif isinstance(obj, Job):
                from forge.api.serializers import JobSerializer
                serializer_class = JobSerializer
            elif isinstance(obj, AdHocCommand):
                from forge.api.serializers import AdHocCommandSerializer
                serializer_class = AdHocCommandSerializer
            elif isinstance(obj, SystemJob):
                from forge.api.serializers import SystemJobSerializer
                serializer_class = SystemJobSerializer
            elif isinstance(obj, WorkflowJob):
                from forge.api.serializers import WorkflowJobSerializer
                serializer_class = WorkflowJobSerializer
            elif isinstance(obj, WorkflowApproval):
                from forge.api.serializers import WorkflowApprovalSerializer
...
class UnifiedJobListSerializer(UnifiedJobSerializer):
    ...
    def get_sub_serializer(self, obj):
        """
        Returns the appropriate list serializer class for a polymorphic object.
        Uses lazy imports to avoid circular dependencies.
        """
        serializer_class = None
        if type(self) is UnifiedJobListSerializer:
            # Lazy imports to avoid circular dependencies
            if isinstance(obj, ProjectUpdate):
                from forge.api.serializers import ProjectUpdateListSerializer
                serializer_class = ProjectUpdateListSerializer
            elif isinstance(obj, InventoryUpdate):
                from forge.api.serializers import InventoryUpdateListSerializer
                serializer_class = InventoryUpdateListSerializer
            elif isinstance(obj, Job):
                from forge.api.serializers import JobListSerializer
                serializer_class = JobListSerializer
            elif isinstance(obj, AdHocCommand):
                from forge.api.serializers import AdHocCommandListSerializer
                serializer_class = AdHocCommandListSerializer
            elif isinstance(obj, SystemJob):
                from forge.api.serializers import SystemJobListSerializer
                serializer_class = SystemJobListSerializer
            elif isinstance(obj, WorkflowJob):
                from forge.api.serializers import WorkflowJobListSerializer
                serializer_class = WorkflowJobListSerializer
            elif isinstance(obj, WorkflowApproval):
                from forge.api.serializers import WorkflowApprovalListSerializer
