...
# AWX
from forge import __version__ as awx_application_version
from forge.main.access import access_registry
...
from forge.main.dispatch import get_task_queuename, reaper
from forge.main.utils.common import ignore_inventory_computed_fields, ignore_inventory_group_removal

from forge.main.utils.reload import stop_local_services
from forge.main.utils.pglock import advisory_lock
from forge.main.tasks.helpers import is_run_threshold_reached
from forge.main.tasks.receptor import get_receptor_ctl, worker_info, worker_cleanup, administrative_workunit_reaper, write_receptor_config
from forge.main.consumers import emit_channel_notification
from forge.main import analytics
from forge.conf import settings_registry
from forge.main.analytics.subsystem_metrics import DispatcherMetrics

from rest_framework.exceptions import PermissionDenied

logger = logging.getLogger('forge.main.tasks.system')
...
def dispatch_startup():
    startup_logger = logging.getLogger('forge.main.tasks')
...
def profile_sql(threshold=1, minutes=1):
    if threshold <= 0:
        cache.delete('forge-profile-sql-threshold')
        logger.error('SQL PROFILING DISABLED')
    else:
        cache.set('forge-profile-sql-threshold', threshold, timeout=minutes * 60)
...
def inspect_established_receptor_connections(mesh_status):
    ...
    '''
    from forge.main.models import InstanceLink
...
def awx_k8s_reaper():
    ...
    from forge.main.scheduler.kubernetes import PodManager  # prevent circular import
...
def deep_copy_model_obj(model_module, model_name, obj_pk, new_obj_pk, user_pk, permission_check_func=None):
    ...
    from forge.api.generics import CopyAPIView
    from forge.main.signals import disable_activity_stream
