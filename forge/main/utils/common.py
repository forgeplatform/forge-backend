...
logger = logging.getLogger('forge.main.utils')
...
def is_testing(argv=None):
    ...
    if os.environ.get('DJANGO_SETTINGS_MODULE') == 'forge.main.tests.settings_for_test':
        return True
    argv = sys.argv if argv is None else argv
    if len(argv) >= 1 and ('py.test' in argv[0] or 'py/test.py' in argv[0] or 'pytest' in argv[0]):
        return True
...
def get_awx_version():
    ...
    try:
        ...
        return pkg_resources.require('forge')[0].version
    except Exception:
        return __version__
...
class ScheduleTaskManager(ScheduleManager):
    def __init__(self):
        from forge.main.scheduler.tasks import task_manager
...
class ScheduleDependencyManager(ScheduleManager):
    def __init__(self):
        from forge.main.scheduler.tasks import dependency_manager
...
class ScheduleWorkflowManager(ScheduleManager):
    def __init__(self):
        from forge.main.scheduler.tasks import workflow_manager
...
def truncate_stdout(stdout, size):
    from forge.main.constants import ANSI_SGR_PATTERN
...
def cleanup_new_process(func):
    ...
    def wrapper_cleanup_new_process(*args, **kwargs):
        from forge.conf.settings import SettingsWrapper  # noqa
