...
# AWX
from forge.main.models import Job, AdHocCommand, ProjectUpdate, InventoryUpdate, SystemJob, WorkflowJob, Notification
from forge.main.utils import unified_job_class_to_event_table_name
...
class Command(BaseCommand):
    ...
    def init_logging(self):
        ...
        self.logger = logging.getLogger('forge.main.commands.cleanup_jobs')
    ...
    def _batch_delete_by_pks(self, model_class, pk_list):
        """Delete objects in batches by PK list."""
        deleted = 0
        for i in range(0, len(pk_list), self.batch_size):
            batch = pk_list[i : i + self.batch_size]
            _, results = model_class.objects.filter(pk__in=batch).delete()
            deleted += results.get(model_class._meta.label, 0)
        return deleted
    ...
    def cleanup_ad_hoc_commands(self):
        ...
        if not self.dry_run:
            ...
            self._batch_delete_by_pks(AdHocCommand, pk_list)
    ...
    def cleanup_project_updates(self):
        ...
        if not self.dry_run:
            ...
            self._batch_delete_by_pks(ProjectUpdate, pk_list)
    ...
    def cleanup_inventory_updates(self):
        ...
        if not self.dry_run:
            ...
            self._batch_delete_by_pks(InventoryUpdate, pk_list)
    ...
    def cleanup_management_jobs(self):
        ...
        if not self.dry_run:
            ...
            self._batch_delete_by_pks(SystemJob, pk_list)
    ...
    def cleanup_workflow_jobs(self):
        ...
        for workflow_job in workflow_jobs.iterator():
            ...
            if workflow_job.status in ('pending', 'waiting', 'running'):
                ...
                if not self.dry_run:
                    pk_list.append(workflow_job.pk)
                deleted += 1

        if not self.dry_run:
            self._batch_delete_by_pks(WorkflowJob, pk_list)
    ...
    def cleanup_notifications(self):
        ...
        for notification in notifications.iterator():
            ...
            if notification.status in ('pending',):
                ...
                if not self.dry_run:
                    pk_list.append(notification.pk)
                deleted += 1

        if not self.dry_run:
            self._batch_delete_by_pks(Notification, pk_list)
