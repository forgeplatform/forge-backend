...
from forge.main.models import accepts_json, UnifiedJobTemplate, UnifiedJob
from forge.main.models.notifications import NotificationTemplate, JobNotificationMixin
from forge.main.models.base import CreatedModifiedModel, VarsDictProperty
from forge.main.models.rbac import ROLE_SINGLETON_SYSTEM_ADMINISTRATOR, ROLE_SINGLETON_SYSTEM_AUDITOR
from forge.main.fields import ImplicitRoleField, JSONBlob, OrderedManyToManyField
...
from forge.main.models.jobs import LaunchTimeConfigBase, LaunchTimeConfig, JobTemplate
from forge.main.models.credential import Credential
...
logger = logging.getLogger('forge.main.models.workflow')
...
class WorkflowJobTemplate(UnifiedJobTemplate, WorkflowJobOptions, SurveyJobTemplateMixin, ResourceMixin, RelatedJobsMixin, WebhookTemplateMixin):
    ...
    def notification_templates(self):
        ...
        approval_q = Q(workflowjobtemplate_notification_templates_for_approvals__in=[self])
        if self.organization is not None:
            error_q |= Q(organization_notification_templates_for_errors=self.organization)
            started_q |= Q(organization_notification_templates_for_started=self.organization)
            success_q |= Q(organization_notification_templates_for_success=self.organization)
            approval_q |= Q(organization_notification_templates_for_approvals=self.organization)
        return dict(
            ...
            approvals=list(base_qs.filter(approval_q).distinct()),
        )
...
class WorkflowApproval(UnifiedJob, JobNotificationMixin):
    ...
    def send_approval_notification(self, approval_status):
        from forge.main.tasks.system import send_notifications  # avoid circular import
