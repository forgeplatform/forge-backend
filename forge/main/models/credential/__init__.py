...
from forge.main.utils import decrypt_field, classproperty, set_environ
from forge.main.utils.safe_yaml import safe_dump
from forge.main.utils.execution_environments import to_container_path
from forge.main.validators import validate_ssh_private_key
from forge.main.models.base import CommonModelNameNotUnique, PasswordFieldsModel, PrimordialModel
from forge.main.models.mixins import ResourceMixin
from forge.main.models.rbac import (
    ROLE_SINGLETON_SYSTEM_ADMINISTRATOR,
    ROLE_SINGLETON_SYSTEM_AUDITOR,
)
from forge.main.models import Team, Organization
from forge.main.utils import encrypt_field
from . import injectors as builtin_injectors

__all__ = ['Credential', 'CredentialType', 'CredentialInputSource', 'build_safe_env']

logger = logging.getLogger('forge.main.models.credential')
credential_plugins = dict((ep.name, ep.load()) for ep in iter_entry_points('forge.credential_plugins'))
...
class Credential(PasswordFieldsModel, CommonModelNameNotUnique, ResourceMixin):
    ...
    @cached_property
    def _cached_input_sources(self):
        if not self.id:
            return []
        return list(self.input_sources.all())

    @cached_property
    def dynamic_input_fields(self):
        return [obj.input_field_name for obj in self._cached_input_sources]
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            # Fetch only the inputs field instead of the full Credential object
            inputs_before = Credential.objects.filter(pk=self.pk).values_list('inputs', flat=True).first()
            if inputs_before:
                ...
                for field in self.PASSWORD_FIELDS:
                    if self.inputs.get(field) == '$encrypted$':
                        self.inputs[field] = inputs_before.get(field, self.inputs[field])
    ...
    def _get_dynamic_input(self, field_name):
        for input_source in self._cached_input_sources:
