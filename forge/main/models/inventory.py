...
from forge.main.constants import CLOUD_PROVIDERS
from forge.main.consumers import emit_channel_notification
from forge.main.fields import (
    ImplicitRoleField,
    SmartFilterField,
    OrderedManyToManyField,
)
from forge.main.managers import HostManager, HostMetricActiveManager
from forge.main.models.base import BaseModel, CommonModelNameNotUnique, VarsDictProperty, CLOUD_INVENTORY_SOURCES, accepts_json
from forge.main.models.events import InventoryUpdateEvent, UnpartitionedInventoryUpdateEvent
from forge.main.models.unified_jobs import UnifiedJob, UnifiedJobTemplate
from forge.main.models.mixins import (
    ResourceMixin,
    TaskManagerInventoryUpdateMixin,
    RelatedJobsMixin,
    CustomVirtualEnvMixin,
)
from forge.main.models.notifications import (
    NotificationTemplate,
    JobNotificationMixin,
)
from forge.main.models.credential.injectors import _openstack_data
from forge.main.utils import _inventory_updates
from forge.main.utils.safe_yaml import sanitize_jinja
from forge.main.utils.execution_environments import to_container_path, get_control_plane_execution_environment
from forge.main.utils.licensing import server_product_name
...
logger = logging.getLogger('forge.main.models.inventory')
...
class Inventory(CommonModelNameNotUnique, ResourceMixin, RelatedJobsMixin):
    ...
    # DEPRECATED: Fields below maintained for backward compat. Removal tracked for future major version.
    ...
    def get_absolute_url(self, request=None):
        if request is not None:
            # circular import
            from forge.api.urls.inventory import constructed_inventory_urls
    ...
    def get_group_hosts_map(self):
        ...
        """
        # NOTE: Computed per-request, caching not needed.
    ...
    def update_computed_fields(self):
        ...
        active_inv_sources_count = active_inventory_sources.count()
        computed_fields = {
            ...
            'has_inventory_sources': bool(active_inv_sources_count),
            'total_inventory_sources': active_inv_sources_count,
    ...
    def schedule_deletion(self, user_id=None):
        from forge.main.tasks.system import delete_inventory
        from forge.main.signals import activity_stream_delete
    ...
    def _update_host_smart_inventory_memeberships(self):
        if self.kind == 'smart' and settings.AWX_REBUILD_SMART_MEMBERSHIP:
            ...
            def on_commit():
                from forge.main.tasks.system import update_host_smart_inventory_memberships
...
class Group(CommonModelNameNotUnique, RelatedJobsMixin):
    ...
    def delete_recursive(self):
        ...
        from forge.main.signals import disable_activity_stream, activity_stream_delete
    ...
    def job_host_summaries(self):
        from forge.main.models.jobs import JobHostSummary

        return JobHostSummary.objects.filter(host__in=self.all_hosts)

    @property
    def job_events(self):
        from forge.main.models.jobs import JobEvent

        return JobEvent.objects.filter(host__in=self.all_hosts)

    @property
    def ad_hoc_commands(self):
        from forge.main.models.ad_hoc_commands import AdHocCommand
...
class HostMetricSummaryMonthly(models.Model):
    """
    HostMetric summaries computed by scheduled task 'forge.main.tasks.system.host_metric_summary_monthly' monthly
    """
...
class InventorySourceOptions(BaseModel):
    ...
    def get_cloud_credential(self, _credentials=None):
        """Return the credential which is directly tied to the inventory source type."""
        credential = None
        credentials = _credentials if _credentials is not None else self.credentials.all()
    ...
    def get_extra_credentials(self):
        ...
        """
        credentials = list(self.credentials.all())
        special_cred = self.get_cloud_credential(_credentials=credentials) if self.source in CLOUD_PROVIDERS else None
        return [cred for cred in credentials if special_cred is None or cred.pk != special_cred.pk]
...
class InventorySource(UnifiedJobTemplate, InventorySourceOptions, CustomVirtualEnvMixin, RelatedJobsMixin):
    ...
    def notification_templates(self):
        base_qs = NotificationTemplate.objects
        org = self.inventory.organization
        error_q = Q(unifiedjobtemplate_notification_templates_for_errors__in=[self])
        started_q = Q(unifiedjobtemplate_notification_templates_for_started__in=[self])
        success_q = Q(unifiedjobtemplate_notification_templates_for_success__in=[self])
        if org is not None:
            error_q |= Q(organization_notification_templates_for_errors=org)
            started_q |= Q(organization_notification_templates_for_started=org)
            success_q |= Q(organization_notification_templates_for_success=org)
        return dict(
            error=list(base_qs.filter(error_q).distinct()),
            started=list(base_qs.filter(started_q).distinct()),
            success=list(base_qs.filter(success_q).distinct()),
        )
...
class InventoryUpdate(UnifiedJob, InventorySourceOptions, JobNotificationMixin, TaskManagerInventoryUpdateMixin, CustomVirtualEnvMixin):
    ...
    def _get_task_class(cls):
        from forge.main.tasks.jobs import RunInventoryUpdate
...
class PluginFileInjector(object):
    ...
    def _get_shared_env(self, inventory_update, private_data_dir, private_data_files):
        ...
        if self.base_injector == 'managed':
            from forge.main.models.credential import injectors as builtin_injectors
