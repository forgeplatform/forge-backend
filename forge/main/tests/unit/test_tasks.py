...
from forge.main.models.credential import HIDDEN_PASSWORD, ManagedCredentialType

from forge.main.tasks import jobs, system, receptor
from forge.main.utils import encrypt_field, encrypt_value
from forge.main.utils.safe_yaml import SafeLoader
from forge.main.utils.execution_environments import CONTAINER_ROOT

from forge.main.utils.licensing import Licenser
...
def mock_create_partition():
    with mock.patch('forge.main.tasks.jobs.create_partition') as cp_mock:
        yield cp_mock
...
def test_send_notifications_job_id(mocker):
    mocker.patch('forge.main.models.UnifiedJob.objects.get')
    system.send_notifications([], job_id=1)
    assert UnifiedJob.objects.get.called
    UnifiedJob.objects.get.assert_called_with(id=1)


@mock.patch('forge.main.models.UnifiedJob.objects.get')
@mock.patch('forge.main.models.Notification.objects.filter')
...
class TestGenericRun:
    def test_generic_failure(self, patch_Job, execution_environment, mock_me, mock_create_partition):
        ...
        with mock.patch('forge.main.tasks.jobs.shutil.copytree'):
            with pytest.raises(Exception):
                task.run(1)

        update_model_call = task.update_model.call_args[1]
        assert 'OSError' in update_model_call['result_traceback'] or 'FileNotFoundError' in update_model_call['result_traceback']
    ...
    def test_awx_task_env(self, patch_Job, private_data_dir, execution_environment, mock_me):
        ...
        with mock.patch('forge.main.tasks.jobs.settings.AWX_TASK_ENV', {'FOO': 'BAR'}):
...
def test_acquire_lock_open_fail_logged(logging_getLogger, os_open, os_path_exists, mock_me):
    err = OSError()
    err.errno = 3
...
def test_acquire_lock_acquisition_fail_logged(fcntl_lockf, logging_getLogger, os_close, os_open, os_path_exists, mock_me):
    err = IOError()
    err.errno = 3
    err.strerror = 'dummy message'
...
def test_administrative_workunit_reaper(work_unit_data, expected_function_call):
    ...
    with mock.patch('forge.main.tasks.receptor.get_receptor_ctl') as mock_get_receptor_ctl:
