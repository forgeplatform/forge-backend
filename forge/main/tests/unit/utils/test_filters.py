...
# AWX
from forge.main.utils.filters import SmartFilter, ExternalLoggerEnabled
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'enabled_flag': False}, 'forge.main', False),
        # skip all records if the host is undefined
        ({'enabled_flag': True}, 'forge.main', False),
        # skip all records if underlying logger is used by handlers themselves
        ({'enabled_flag': True}, 'forge.main.utils.handlers', False),
        ({'enabled_flag': True, 'enabled_loggers': ['forge']}, 'forge.main', True),
        ({'enabled_flag': True, 'enabled_loggers': ['abc']}, 'forge.analytics.xyz', False),
        ({'enabled_flag': True, 'enabled_loggers': ['xyz']}, 'forge.analytics.xyz', True),
    ],
)
...
@mock.patch('forge.main.utils.filters.get_model', return_value=mockHost())
