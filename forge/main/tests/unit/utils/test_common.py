...
from forge.main.utils import common
from forge.api.validators import HostnameRegexValidator

from forge.main.models import Job, AdHocCommand, InventoryUpdate, ProjectUpdate, SystemJob, WorkflowJob, Inventory, JobTemplate, UnifiedJobTemplate, UnifiedJob
...
def memoized_function(mocker, mock_cache):
    with mock.patch('forge.main.utils.common.get_memoize_cache', return_value=mock_cache):
...
def test_memoize_delete(memoized_function, mock_cache):
    ...
    with mock.patch('forge.main.utils.common.memoize_delete', side_effect=mock_cache.delete):
